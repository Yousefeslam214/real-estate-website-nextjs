...
import { MapPin, Bed, Bath, Square, Calendar, Phone, Mail, Heart, Share2, Camera, Car, Wifi, Dumbbell, Shield, TreePine, Building } from 'lucide-react';
...
const PropertyDetailsPage: React.FC = () => {
  ...
  const property = {
    id: 'E101847',
    title: language === 'ar' ? 'شقة فاخرة في العلمين - الحي اللاتيني' : 'Luxury Apartment in Alamain - Latin District',
    location: language === 'ar' ? 'العلمين، الساحل الشمالي، الإسكندرية' : 'Alamain, North Coast, Alexandria',
    fullAddress: language === 'ar' ? 'مبنى E03، الطابق الأرضي، الوحدة Z03-CL07-E03-X4-00-03' : 'Building E03, Floor G, Unit Z03-CL07-E03-X4-00-03',
    price: 4500000,
    area: 180,
    bedrooms: 3,
    bathrooms: 2,
  ...
  const getFeatureIcon = (feature: string) => {
    ...
    if (lowerFeature.includes('car') || lowerFeature.includes('موقف')) return Car;
    if (lowerFeature.includes('wifi') || lowerFeature.includes('إنترنت')) return Wifi;
    if (lowerFeature.includes('gym') || lowerFeature.includes('رياضية')) return Dumbbell;
    if (lowerFeature.includes('security') || lowerFeature.includes('أمن')) return Shield;
    if (lowerFeature.includes('garden') || lowerFeature.includes('حديقة')) return TreePine;
    return Building;
  };
              ...
              <div className="bg-white rounded-2xl shadow-lg p-8">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-sm text-gray-500">
                      {Math.round(property.price / property.area).toLocaleString()} {language === 'ar' ? 'ج.م/م²' : 'EGP/m²'}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                  <div className="text-center p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-sm text-gray-600">{language === 'ar' ? 'متر مربع' : 'Square Meters'}</div>
                  </div>
                  <div className="text-center p-4 bg-gray-50 rounded-lg">
                    <Bed className="h-8 w-8 mx-auto mb-2 text-green-600" />
                    <div className="text-2xl font-bold text-gray-900">{property.bedrooms}</div>
                    <div className="text-sm text-gray-600">{language === 'ar' ? 'غرف نوم' : 'Bedrooms'}</div>
                  </div>
                  <div className="text-center p-4 bg-gray-50 rounded-lg">
                    <Bath className="h-8 w-8 mx-auto mb-2 text-purple-600" />
                    <div className="text-2xl font-bold text-gray-900">{property.bathrooms}</div>
                    <div className="text-sm text-gray-600">{language === 'ar' ? 'حمامات' : 'Bathrooms'}</div>
                  </div>
                  <div className="text-center p-4 bg-gray-50 rounded-lg">
                    <Calendar className="h-8 w-8 mx-auto mb-2 text-orange-600" />
                    <div className="text-2xl font-bold text-gray-900">{property.yearBuilt}</div>
                    <div className="text-sm text-gray-600">{language === 'ar' ? 'سنة البناء' : 'Year Built'}</div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg p-8">
                ...
                <div className="bg-gray-200 rounded-xl h-64 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-gray-500">Alamain, North Coast, Alexandria</p>
              ...
              <div className="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div className="flex items-center space-x-4 rtl:space-x-reverse mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{language === 'ar' ? 'وكيل عقاري معتمد' : 'Certified Real Estate Agent'}</p>
                  </div>
                </div>
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <span>{language === 'ar' ? 'إرسال رسالة' : 'Send Email'}</span>
                  </a>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    <span className="text-gray-600">{language === 'ar' ? 'نوع العقار:' : 'Property Type:'}</span>
                    <span className="font-medium">{property.propertyType}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">{language === 'ar' ? 'الطابق:' : 'Floor:'}</span>
                    <span className="font-medium">{property.floor === 0 ? (language === 'ar' ? 'الطابق الأرضي' : 'Ground Floor') : property.floor}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">{language === 'ar' ? 'إجمالي الطوابق:' : 'Total Floors:'}</span>
                    <span className="font-medium">{property.totalFloors}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">{language === 'ar' ? 'سنة البناء:' : 'Year Built:'}</span>
                    <span className="font-medium">{property.yearBuilt}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">{language === 'ar' ? 'رقم العقار:' : 'Property ID:'}</span>
